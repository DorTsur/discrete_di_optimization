...
class DVEncoderVisualizer(DVVisualizer):
    def __init__(self, config):
        super().__init__(config)
        self.x_list = list()
        self.y_list = list()
    ...
    def update_state(self, t_y, t_xy, x, y, p):
    ...
    def convert_lists_to_np(self):
        ...
        y_n = [y[0] for y in self.y_list]
        y_np = tf.concat(y_n, axis=1)
        p_n = [p[0] for p in self.p_list]
        p_np = tf.concat(p_n, axis=1)
        return t_y, t_y_, t_xy, t_xy_, x_np, y_np, p_np

    def save(self, name=None):

        t_y, t_y_, t_xy, t_xy_, x, y, p = self.convert_lists_to_np()
...
class DVEncoderVisualizer_states(DVVisualizer):
    def __init__(self, config):
        super().__init__(config)
        self.x_list = list()
        self.y_list = list()
        self.p_list = list()
        self.states_list = list()
        self.fsc_flag = (config.channel_name in ["trapdoor","ising","GE","post"])
        if self.fsc_flag:
    ...
    def save(self, models=None, name=None):
        ...
        savemat(os.path.join(self.config.tensor_board_dir, 'visual',
                             file_name), {"t_y": t_y.numpy(),
                                          ...
                                          "p": p.numpy(),
                                          "states": states.numpy()})
        path = os.path.join(self.config.tensor_board_dir,"models_weights")
        self.save_models(models, path)
        if self.fsc_flag:
            self.q_hist.plot(save=True, save_path=self.config.tensor_board_dir, save_name="est_Q_hist.png")
...
class PDINE_no_q_visualizer(DVVisualizer):
    def __init__(self, config):
        super().__init__(config)
        self.x_list = list()
        self.y_list = list()
...
class pdine_saver(Visualizer):
    def __init__(self, config):
        super().__init__(config)
        self.dv_states_flag = config.compress_dv_flag
        self.keys = ["x", "y", "p", "t_y", "t_xy", "states_t_y"]
        self.data = {}
        self.np_data = {}
        self.reset_state()
    ...
    def update_state(self,data):
        ...
        # self.data["x"].append(data["x"])
        # self.data["y"].append(data["y"])
        # self.data["p"].append(data["p"])
        # self.data["t_y"].append(data["t_y"])
        # self.data["t_xy"].append(data["t_xy"])
        # self.data["t_y_states"].append(data["t_y_states"])

    def convert_lists_to_np(self):
        for key in self.keys:
            if key == "t_y" or key == "t_xy":
                self.np_data[key+"_0"] = tf.concat([d[0] for d in self.data[key]],axis=1)
                self.np_data[key + "_0"] = self.np_data[key+"_0"].numpy()
                self.np_data[key+"_1"] = tf.concat([d[1] for d in self.data[key]], axis=1)
                self.np_data[key + "_1"] = self.np_data[key+"_1"].numpy()

            else:
                if key == "states_t_y" and not self.dv_states_flag:
                    continue
                self.np_data[key] = tf.concat([d for d in self.data[key]], axis=1)
                self.np_data[key] = self.np_data[key].numpy()


    def save(self, name=None):
        self.convert_lists_to_np()
        file_name = name if name is not None else 'raw_data_latest.mat'
        savemat(os.path.join(self.config.tensor_board_dir, 'visual',
                             file_name), self.np_data)
...
class ContDINESaver(DVVisualizer):
    def __init__(self, config):
        super().__init__(config)
        self.x_list = list()
        self.y_list = list()
    ...
    def update_state(self, t_y, t_xy, x, y):
        super().update_state(t_y, t_xy)
        self.x_list.append(x)
        self.y_list.append(y)
